...
class ContrastiveMemoryBank(nn.Module):
    def __init__(self, feature_dim, queue_size=1024):
        super().__init__()
    ...
    def enqueue(self, keys):
        ...
        if self.ptr + batch_size > self.queue_size:
            self.queue[:, self.ptr:] = keys[:self.queue_size - self.ptr].T
            self.queue[:, :batch_size - (self.queue_size - self.ptr)] = keys[self.queue_size - self.ptr:].T
            self.ptr = (self.ptr + batch_size) % self.queue_size
        else:
            self.queue[:, self.ptr:self.ptr + batch_size] = keys.T
            self.ptr = (self.ptr + batch_size) % self.queue_size

def contrastive_loss(query_features, groups, memory_bank, temperature=0.07):
    """
    计算一批查询特征的对比损失。

    参数:
        query_features (Tensor): 形状为 (N, D) 的张量，其中 N 是查询的数量，D 是每个特征向量的维度。
        groups (List[List[int]]): 列表的列表，每个子列表包含属于同一组（正样本对）的查询特征的索引。
        memory_bank (MemoryBank): MemoryBank 实例，包含一队负样本。
        temperature (float, optional): logits 的缩放因子。默认值为 0.07。

    返回:
        Tensor: 计算得到的对比损失。
    """
    # 规范化特征
    query_features = F.normalize(query_features, dim=1) # 确保每个特征向量的长度为1。归一化后的特征可以简化相似性计算，因为余弦相似度与点积相等。
    memory = memory_bank.queue.clone().detach()  # (D, K)
    
    loss = 0.0
    for group in groups:
        if len(group) < 2:
            continue  # 需要至少两个正样本
        positives = query_features[group]  # (P, D) 组内有P个向量
        negatives = memory.T  # (K, D)
        
        logits_pos = positives @ positives.t()  # (P, P) 组内两两之间的相似度
        logits_neg = torch.mm(positives, negatives)  # (P, K) 组内向量与组外的K个向量的相似度
        
        labels = torch.arange(positives.size(0)).to(query_features.device)  # (P,)
        logits = torch.cat([logits_pos, logits_neg], dim=1)  # (P, P + K)
        labels = labels.to(query_features.device)
...
def contrastive_loss_(query_features, groups, memory_bank, temperature=0.07):
    """
    计算一批查询特征的对比损失，每个查询样本有一个正样本和多个负样本。
    ...
    """
    # 规范化特征
    query_features = F.normalize(query_features, dim=1)  # (N, D)
    memory = memory_bank.queue.clone().detach()  # (D, K)
    memory = F.normalize(memory, dim=1)  # 规范化负样本
    ...
    for group in groups:
        ...
        # 假设每个组有多个正样本，可以将一个作为查询，其他作为正样本
        # 这里我们将第一个作为查询，其他作为正样本
        query = query_features[group[0]].unsqueeze(0)  # (1, D)
        positives = query_features[group[1:]]  # (P-1, D)

        # 计算与正样本的相似度，并取平均作为正样本的相似度
        sim_pos = torch.matmul(query, positives.t()).mean(1, keepdim=True)  # (1, 1)

        # 计算与负样本的相似度
        sim_neg = torch.matmul(query, memory.t())  # (1, K)

        # 拼接相似度
        logits = torch.cat([sim_pos, sim_neg], dim=1)  # (1, 1 + K)

        # 标签：第一个是正样本
        label = torch.tensor([0]).to(query_features.device)  # (1,)
    ...
    if total_pairs == 0:
        return torch.tensor(0.0).to(query_features.device)

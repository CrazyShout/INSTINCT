...
class DeformableTransformerBackbone(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        record_len = data_dict['record_len'] # [n1, n2]
        ...
        pairwise_t_matrix = pairwise_t_matrix[:,:,:,[0, 1],:][:,:,:,:,[0, 1, 3]] # [B_n, L, L, 2, 3]
        ...
        if self.model_cfg['fusion'].startswith('deform'):
            ...
            x_fuseds = [self.fuse_net[i](features[i], record_len, pairwise_t_matrix) for i in range(len(features))] # [(B, C, H/2, W/2), (B, 2C, H/4, W/4), (B, 4C, H/8, W/8)]

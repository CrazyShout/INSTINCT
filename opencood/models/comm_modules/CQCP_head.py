...
from opencood.models.sub_modules.TrasIFF_transformer import Transformer, MLP, get_clones
    ...
    def predict(self, batch_dict):
        ...
        if self.train_flag and dn['enabled'] and dn['dn_number'] > 0:
            ...
            input_query_label, input_query_bbox, attn_mask, dn_meta = prepare_for_cdn(
                ...
                num_queries=self.num_queries+200, # 对齐，不然没法concat
        ...
        outputs = {
            ...
            "pred_logits": outputs_class[-1][:, : self.num_queries + 200],    # (B, all_query_num, 1) # 最后一层
            "pred_boxes": outputs_coord[-1][:, : self.num_queries + 200],     # (B, all_query_num, 7)
            "aux_outputs": self._set_aux_loss(
                outputs_class[:-1, :, : self.num_queries+200], outputs_coord[:-1, :, : self.num_queries+200],  # List[Dict{"pred_logits": (B, 1000, 3), "pred_boxes": (B, 1000, 7), ...] 5个元素 表示前五层的1000个query
            ),
        }
        if self.train_flag:
            '''
        ...
        if dn_meta is not None:
            ...
            if num_gts > 0:
                for li in range(self.model_cfg["num_decoder_layers"]): # 3层decoder
                    ...
                    gt_projs = self.projector(projs[:, self.num_queries+200:].detach()) # gt 线性变化 # (B, 4*max_gt_num, 256) 注意这四批的gt前一批是真gt，后面三个则是噪声正样本 注意，这里detach是因为这里不能梯度回传，这个是将其映射到高维空间
                    pred_projs = self.predictor(self.projector(projs[:, : self.num_queries+200])) # (B, all_query_num, 256) query需要额外的MLP，出自ConQueR论文中的设计
                    # num_gts x num_locs
                    ...
                    for bi, idx in enumerate(outputs["matched_indices"]): # 这个是[((n1,), (n1,)), ...]匹配结果
                        ...
                        neg_mask = projs.new_ones(self.num_queries+200).bool() # (all_query_num, )
